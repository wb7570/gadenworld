package tools {
	...
	public class LoaderTool {
		...
		private var _handlerVect:Vector.<MethodHandler> = new Vector.<MethodHandler>();
		private var _lHandler:LoaderHandler;
		...
		public function LoaderTool(applicationDomain:ApplicationDomain = null) {
		...
		public function loadSWF(file:String,func:Function=null,args:Array=null):void {
			load(SWF,file,func,args);
		}
		
		public function loadIMG(file:String,func:Function=null,args:Array=null):void {
			load(IMG,file,func,args);
		}
		
		public function loadBMD(file:String,func:Function=null,args:Array=null):void {
			load(BMD,file,func,args);
		}
		
		public function loadAMF(file:String,func:Function=null,args:Array=null):void {
			load(AMF,file,func,args);
		}
		public function loadAMFObj(file:String,func:Function=null,args:Array=null):void{
			load(AMF_OBJ,file,func,args);
		}
		public function loadText(file:String,func:Function=null,args:Array=null):void
		{
			load(TEXT,file,func,args);
		}
		
		public function load(type:int, file:String, func:Function = null, args:Array = null,context:LoaderContext=null):void {
			if (file == null) return;
			var handler:LoaderHandler=new LoaderHandler();
			handler.type=type;
			handler.file=file;
			handler.func=func;
			handler.args = args;
			...
			var cache:Object=_mCache[handler.file];
			if (cache!=null) {
		...
		{
			var cache:Object = _mCache[_lHandler.file];
			if (cache!=null) {
			...
			if (_lHandler.type==SWF) {
				_loader.load(getRequest(_lHandler.file),_lHandler.context ? _lHandler.context : _context);
				return;
			}
			if (_lHandler.type==IMG) {
				_loader.load(getRequest(_lHandler.file));
				return;
			}
			if (_lHandler.type==BMD) {
				if (useBitmapDataHack) {
					_urlLoader.dataFormat=URLLoaderDataFormat.BINARY;
					_urlLoader.load(getRequest(_lHandler.file));
				}
				else {
					
					_loader.load(getRequest(_lHandler.file), getLoaderDomain());
			...
			if (_lHandler.type==AMF_OBJ) {
				...
				var oRequest:URLRequest=new URLRequest(_lHandler.file);
		...
		private function onLoadComplete(event:Event):void {
			if (_lHandler.type==SWF) {
				_lHandler.executeWith(_mCache[_lHandler.file]=_loader.content);
				checkNext();
				return;
			}
			if (_lHandler.type==IMG) {
			...
			if (_lHandler.type==BMD) {
				...
				function onCom(event:Event):void
				{
					...
					_lHandler.executeWith(_mCache[_lHandler.file] = bitmapData);
					checkNext();
				}
			...
			if (_lHandler.type == TEXT) {
				_lHandler.executeWith(_mCache[_lHandler.file]=_urlLoader.data);
				checkNext();
				return;
			}
		}
		
		static private var _main:LoaderTool = new LoaderTool();
		
		static public function load(type:int, file:String, func:Function = null, args:Array = null,context:LoaderContext=null):void
		{
			_main.load(type,file,func,args,context);
		}
	}
}
